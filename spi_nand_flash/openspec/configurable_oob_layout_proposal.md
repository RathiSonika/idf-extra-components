...
| Q3 | **`MAX_REG` / `SPI_NAND_OOB_MAX_REGIONS` for v1:** locked to **8** (see [`changes/configurable-oob-layout/README.md`](changes/configurable-oob-layout/README.md) *Implementation decisions*). **Status:** implemented as `#define SPI_NAND_OOB_MAX_REGIONS 8` (step **02**). Revisit only if a datasheet-backed layout needs more than eight disjoint free fragments. |
| Q4 | Are **RFC public APIs** (`spi_nand_oob_field_read/write`, raw OOB) intended to be **stable** under `include/` or **internal/debug-only** for the first release? **Decision:** stays **private** under `priv_include/` for this release; promotion is a separate change (see baseline §4.0 stability contract). **Status:** unchanged for the experimental OOB v1 rollout (step **12**). |
| Q8 | **Known bugs vs this epic:** [**§11.4.2**](known-bugs.md) (device handle + Dhara private struct to internal RAM) — fixed in [`step 05`](changes/configurable-oob-layout/step-05-device-state-and-init-hook.md). [**§11.4.1**](known-bugs.md) (`nand_emul_get_stats`) — fixed in [`step 10`](changes/configurable-oob-layout/step-10-linux-parity.md). `known-bugs.md` / baseline §11.4 are synced when those steps are merged. Layout metadata stays inside the handle object (no separate heap blob for layout-only state). |
