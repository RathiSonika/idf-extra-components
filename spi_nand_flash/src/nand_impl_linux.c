...
esp_err_t nand_is_bad(spi_nand_flash_device_t *handle, uint32_t block, bool *is_bad_status)
{
    ...
    ESP_GOTO_ON_ERROR(nand_emul_read(handle, block_offset + handle->chip.page_size, (uint8_t *) &bad_block_indicator, 2),
                      fail, TAG, "");
...
esp_err_t nand_mark_bad(spi_nand_flash_device_t *handle, uint32_t block)
{
    ...
    ESP_GOTO_ON_ERROR(nand_emul_erase_block(handle, block * handle->chip.block_size),
                      fail, TAG, "");

    ESP_GOTO_ON_ERROR(nand_emul_write(handle, block * handle->chip.block_size + handle->chip.page_size,
...
esp_err_t nand_erase_block(spi_nand_flash_device_t *handle, uint32_t block)
{
    ...
    ESP_GOTO_ON_ERROR(nand_emul_erase_block(handle, address),
                      fail, TAG, "");
    return ret;
...
esp_err_t nand_prog(spi_nand_flash_device_t *handle, uint32_t page, const uint8_t *data)
{
    ...
    ESP_GOTO_ON_ERROR(nand_emul_write(handle, data_offset, data, handle->chip.page_size), fail, TAG, "");
    ESP_GOTO_ON_ERROR(nand_emul_write(handle, data_offset + handle->chip.page_size + 2,
...
esp_err_t nand_is_free(spi_nand_flash_device_t *handle, uint32_t page, bool *is_free_status)
{
    ...
    ESP_GOTO_ON_ERROR(nand_emul_read(handle, page * handle->chip.emulated_page_size + handle->chip.page_size + 2, (uint8_t *)&used_marker, 2),
                      fail, TAG, "");
...
esp_err_t nand_read(spi_nand_flash_device_t *handle, uint32_t page, size_t offset, size_t length, uint8_t *data)
{
    ...
    ESP_GOTO_ON_ERROR(nand_emul_read(handle, page * handle->chip.emulated_page_size + offset, data, length),
                      fail, TAG, "");
...
esp_err_t nand_copy(spi_nand_flash_device_t *handle, uint32_t src, uint32_t dst)
{
    ...
    ESP_GOTO_ON_ERROR(nand_emul_read(handle, (size_t)src_offset, (void *)handle->read_buffer, handle->chip.page_size),
                      fail, TAG, "");
    ESP_GOTO_ON_ERROR(nand_emul_write(handle, (size_t)dst_offset, (void *)handle->read_buffer, handle->chip.page_size), fail, TAG, "");
