...
- Optional **FAT + VFS on the WL BDL** (ESP-IDF 6.0+): use **`esp_vfs_fat_bdl_mount()`** / **`esp_vfs_fat_bdl_unmount()`** with the wear-leveling handle from **`spi_nand_flash_init_with_layers()`** (FatFS **`diskio_bdl`**). Example: **`spi_nand_flash_fatfs/examples/nand_flash_bdl`** in the `idf-extra-components` repository. The legacy FatFS integration (**`esp_vfs_fat_nand_mount()`** in `spi_nand_flash_fatfs`) still requires **BDL off** and **`spi_nand_flash_init_device()`**.
...
In this mode, “Application / Filesystem” may be your own block-device consumer or a FAT volume: on ESP-IDF 6.0+, mounting FAT on the WL BDL uses **`esp_vfs_fat_bdl_mount()`** (see **`spi_nand_flash_fatfs/examples/nand_flash_bdl`**), which is separate from the legacy **`esp_vfs_fat_nand_*`** helpers in `spi_nand_flash_fatfs`.
