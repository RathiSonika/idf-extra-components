...
2. **BDL + FatFS (ESP-IDF 6.0+, `CONFIG_NAND_FLASH_ENABLE_BDL=y`):** `spi_nand_flash_init_with_layers()` and ESP-IDF’s **`esp_vfs_fat_bdl_mount()`** / **`esp_vfs_fat_bdl_unmount()`** (generic `diskio_bdl`; does not use `esp_vfs_fat_nand_*`). See **`spi_nand_flash_fatfs/examples/nand_flash_bdl`**.
