...
TEST_CASE("erase nand flash", "[spi_nand_flash]")
{
    ...
    TEST_ESP_OK(bdl_handle->erase(bdl_handle, 1, bdl_handle->geometry.block_count));
...
TEST_CASE("verify mark_bad_block works", "[spi_nand_flash]")
{
    ...
    esp_blockdev_handle_t bdl_handle;
    setup_nand_flash(&nand_flash_device_handle, &spi, SPI_NAND_IO_MODE_SIO, SPI_DEVICE_HALFDUPLEX, &bdl_handle);
    uint32_t block_num = bdl_handle->geometry.block_count;
...
static void test_nand_operations(spi_nand_flash_io_mode_t mode, uint8_t flags)
{
    ...
    esp_blockdev_handle_t bdl_handle;
    setup_nand_flash(&nand_flash_device_handle, &spi, SPI_NAND_IO_MODE_SIO, flags, &bdl_handle);
