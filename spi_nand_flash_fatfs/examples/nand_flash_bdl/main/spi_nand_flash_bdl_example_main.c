...
#if CONFIG_EXAMPLE_FORMAT_BEFORE_MOUNT
...
#if CONFIG_EXAMPLE_FORMAT_BEFORE_MOUNT
/**
 * whole-volume format using esp_vfs_fat_format_drive().
 *
 * esp_vfs_fat_bdl_mount() registers its own diskio slot; this path registers temporarily,
 * formats, then unregisters so mount can allocate a fresh drive number.
 */
        ...
        const esp_vfs_fat_mount_config_t *mount_config)
{
    ...
    if (err != ESP_OK) {
        ...
        ESP_LOGI(TAG, "Volume formatted on FatFS drive %u", (unsigned)pdrv);
    }
    return err;
}
#endif
